...
############配置参数
work_dir = "E:\\Code\\stock_compute\\strategy\\factor_verify_res\\热门回撤_all\\"
...
class base_config:
    ...
    end_date = '2022-10-01'
    #回测模式
    mode = {'ratio':0}#{'limit_count':10}#limit_cash:初始资金限制；limit_count:持仓数限制；ratio:累计比值模式；
    #超时
    timeout = 10
    #止盈
...
class out_config:
    #是否存储回测数据表格
    save_result = True
    result_file_path = work_dir + '信号统计结果数据{}.csv'  # single_limit_factors_18-22\\return_18-22_7止盈_{}.csv'
    xinhao_details_file_path = work_dir + '信号明细数据{}.csv'  # single_limit_factors_18-22\\return_18-22_7止盈_{}.csv'
...
class trading:
        ...
        for id, position in self.position_buffer.items():
            ...
            if return_ratio_p >= position.conf.stop_profit:
                ...
                if ((market.open_price / position.buy_price - 1) * 100) > position.conf.stop_profit:
                    ...
                    sell_price = position.buy_price *(1+position.conf.stop_profit/100)
                close_type += '止盈'
            # 止损强平
                ...
                if ((market.open_price / position.buy_price - 1) * 100) < position.conf.stop_loss:
                    ...
                    sell_price = position.buy_price * (1 - position.conf.stop_loss/100)
    ...
    def positions_to_df(self):
        ...
        df.to_csv(out_config.xinhao_details_file_path.format(time), index=False)
        df['count'] = 1
